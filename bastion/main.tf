locals {
  ...
  # landing_zone_state_key = "db/${lower("nprd")}/hub_landing_zone.tfstate"
}
...
resource "azurerm_public_ip" "bastion" {
  ...
  sku                     = "Standard"
  public_ip_prefix_id     = local.public_ip_prefix[each.value].id #data.terraform_remote_state.hub.outputs.hub_public_ip_prefixes[each.value].id
